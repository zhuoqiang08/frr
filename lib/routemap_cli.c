	...
	ROUTE_MAP_SEQUENCE_CMD_STR)
{
	...
	snprintf(xpath, sizeof(xpath),
		 "/frr-route-map:route-map/instance[name='%s']", name);
	...
	ROUTE_MAP_SEQUENCE_CMD_STR)
{
	...
	snprintf(xpath, sizeof(xpath),
		 "/frr-route-map:route-map/instance[name='%s']/entry[sequence='%lu']",
		 name, sequence);
	nb_cli_enqueue_change(vty, xpath, NB_OP_DESTROY, NULL);
...
void route_map_instance_show(struct vty *vty, struct lyd_node *dnode,
			     bool show_defaults)
{
	const struct route_map_rule *rlr;
...
#define SKIP_RULE(name) if (strcmp((name), rlr->cmd->str) == 0) continue

	/* Print route map `match` for old CLI users. */
	for (rlr = rmi->match_list.head; rlr; rlr = rlr->next) {
		...
		vty_out(vty, " match %s %s\n", rlr->cmd->str,
			rlr->rule_str ? rlr->rule_str : "");
	}

	/* Print route map `set` for old CLI users. */
	for (rlr = rmi->set_list.head; rlr; rlr = rlr->next) {
