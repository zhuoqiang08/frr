...
#define PREFIX_LIST_DESC_STR "Prefix-list specific comment\n"
	...
	ACCESS_LIST_REMARK_LINE_STR)
{
	...
	snprintf(xpath_remark, sizeof(xpath_remark), "%s/description", xpath);
	remark = argv_concat(argv, argc, 4);
	nb_cli_enqueue_change(vty, xpath_remark, NB_OP_CREATE, remark);
	rv = nb_cli_apply_changes(vty, NULL);
	XFREE(MTYPE_TMP, remark);
	...
	PREFIX_LIST_DESC_STR)
{
	...
	snprintf(xpath, sizeof(xpath),
		 "/frr-filter:lib/prefix-list[type='ipv4'][name='%s']/description",
		 name);
	nb_cli_enqueue_change(vty, xpath, NB_OP_DESTROY, NULL);
	...
	ACCESS_LIST_REMARK_STR)
{
	...
	snprintf(xpath, sizeof(xpath),
		 "/frr-filter:lib/prefix-list[type='ipv6'][name='%s']/description",
		 name);
	nb_cli_enqueue_change(vty, xpath, NB_OP_DESTROY, NULL);
