			       ...
			       struct stream *s)
{
	...
	/* Real length will be adjust later */
	len_pos = stream_get_endp(s);
	stream_putc(s, tlv_len);
	...
	while (subtlv_len > 0) {
		...
		case ISIS_SUBTLV_SID_LABEL_RANGE:
			...
			if (rc->srgb.range_size == 0
				...
				sbuf_push(log, indent, "Invalid label range\n");
				return 1;
			}
			break;
		case ISIS_SUBTLV_ALGORITHM:
			...
			stream_get(&rc->algo, s,
				   length > SR_ALGORITHM_COUNT ? SR_ALGORITHM_COUNT
							    : length);
...
DEFINE_HOOK(isis_adj_ipv4_enabled_hook, (struct isis_adjacency *adj), (adj))
DEFINE_HOOK(isis_adj_ipv4_disabled_hook, (struct isis_adjacency *adj), (adj))
DEFINE_HOOK(isis_adj_ipv6_enabled_hook, (struct isis_adjacency *adj), (adj))
DEFINE_HOOK(isis_adj_ipv6_disabled_hook, (struct isis_adjacency *adj), (adj))
				       ...
				       bool *changed)
{
	...
	if (adj->ipv4_address_count == 0 && tlvs->ipv4_address.count > 0)
		...
		hook_call(isis_adj_ipv4_disabled_hook, adj);
	...
	if (ipv4_enabled)
		hook_call(isis_adj_ipv4_enabled_hook, adj);
}
				       ...
				       bool *changed)
{
	...
	if (adj->ipv6_address_count == 0 && tlvs->ipv6_address.count > 0)
		...
		hook_call(isis_adj_ipv6_disabled_hook, adj);
	...
	if (ipv6_enabled)
		hook_call(isis_adj_ipv6_enabled_hook, adj);
}
				     ...
				     const struct isis_sr_db *srdb)
{
	...
	tlvs->router_cap->srgb.range_size = srdb->config.srgb_upper_bound
					    - srdb->config.srgb_lower_bound + 1;
	tlvs->router_cap->srgb.lower_bound = srdb->config.srgb_lower_bound;
	tlvs->router_cap->algo[0] = SR_ALGORITHM_SPF;
	tlvs->router_cap->algo[1] = SR_ALGORITHM_UNSET;
	tlvs->router_cap->msd = srdb->config.msd;
}
